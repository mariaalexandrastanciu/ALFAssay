...
pearl_patients_complete <- pearl[complete.cases(pearl[ , 20:21]),] %>% group_by(patient) %>% summarise(count = n() ) %>% filter(count >2)
...
ggsave("plots/PearlTimepoints.pdf",pearl_timepoints,width=15, height=8)
...
neorhea_patients_complete <- neorhea[complete.cases(neorhea[ , 21:22]),] %>% group_by(patient) %>% summarise(count = n() ) %>% filter(count >2)
...
ggsave("plots/NeorheaTimepoints.pdf",neorhea_timepoints,width=15, height=8)
...
ggsave("plots/SynergyTimepoints.pdf",synergy_timepoints,width=15, height=8)
